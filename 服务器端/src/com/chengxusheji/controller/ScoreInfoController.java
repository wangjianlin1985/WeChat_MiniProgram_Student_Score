...
public class ScoreInfoController extends BaseController {
	...
	@InitBinder("courseNo")
	public void initBindercourseNo(WebDataBinder binder) {
		binder.setFieldDefaultPrefix("courseNo.");
	}
	...
	public void list(@ModelAttribute("studentNumber") Student studentNumber,@ModelAttribute("courseNo") Course courseNo,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if(rows != 0)scoreInfoService.setRows(rows);
		List<ScoreInfo> scoreInfoList = scoreInfoService.queryScoreInfo(studentNumber, courseNo, page);
	    /*计算总的页数和总的记录数*/
	    scoreInfoService.queryTotalPageAndRecordNumber(studentNumber, courseNo);
	    /*获取到总的页码数目*/
	...
	public String frontlist(@ModelAttribute("studentNumber") Student studentNumber,@ModelAttribute("courseNo") Course courseNo,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		List<ScoreInfo> scoreInfoList = scoreInfoService.queryScoreInfo(studentNumber, courseNo, currentPage);
	    /*计算总的页数和总的记录数*/
	    scoreInfoService.queryTotalPageAndRecordNumber(studentNumber, courseNo);
	    /*获取到总的页码数目*/
	    int totalPage = scoreInfoService.getTotalPage();
	...
	public void OutToExcel(@ModelAttribute("studentNumber") Student studentNumber,@ModelAttribute("courseNo") Course courseNo, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        List<ScoreInfo> scoreInfoList = scoreInfoService.queryScoreInfo(studentNumber,courseNo);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "ScoreInfo信息记录"; 
        String[] headers = { "成绩编号","考察学生","考察课程","成绩得分","学生评价"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<scoreInfoList.size();i++) {
        	ScoreInfo scoreInfo = scoreInfoList.get(i); 
        	dataset.add(new String[]{scoreInfo.getScoreId() + "",scoreInfo.getStudentNumber().getStudentName(),scoreInfo.getCourseNo().getCourseName(),scoreInfo.getScore() + "",scoreInfo.getEvaluate()});
        }
        /*
