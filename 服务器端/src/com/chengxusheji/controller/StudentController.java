...
public class StudentController extends BaseController {
	...
	@InitBinder("classInfoId")
	public void initBinderclassInfoId(WebDataBinder binder) {
		binder.setFieldDefaultPrefix("classInfoId.");
	}
	@InitBinder("student")
	...
	public void add(@Validated Student student, BindingResult br,
		...
		try {
			student.setPhotoUrl(this.handlePhotoUpload(request, "photoUrlFile"));
	...
	public void list(String studentNumber,@ModelAttribute("classInfoId") ClassInfo classInfoId,String studentName,String birthday,String telephone,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<Student> studentList = studentService.queryStudent(studentNumber, classInfoId, studentName, birthday, telephone, page);
	    /*计算总的页数和总的记录数*/
	    studentService.queryTotalPageAndRecordNumber(studentNumber, classInfoId, studentName, birthday, telephone);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String studentNumber,@ModelAttribute("classInfoId") ClassInfo classInfoId,String studentName,String birthday,String telephone,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (studentNumber == null) studentNumber = "";
		if (studentName == null) studentName = "";
		if (birthday == null) birthday = "";
		if (telephone == null) telephone = "";
		List<Student> studentList = studentService.queryStudent(studentNumber, classInfoId, studentName, birthday, telephone, currentPage);
	    /*计算总的页数和总的记录数*/
	    studentService.queryTotalPageAndRecordNumber(studentNumber, classInfoId, studentName, birthday, telephone);
	    /*获取到总的页码数目*/
	    int totalPage = studentService.getTotalPage();
	    ...
	    request.setAttribute("classInfoId", classInfoId);
	...
	public void update(@Validated Student student, BindingResult br,
		...
		String photoUrlFileName = this.handlePhotoUpload(request, "photoUrlFile");
		if(!photoUrlFileName.equals("upload/NoImage.jpg"))student.setPhotoUrl(photoUrlFileName); 
	...
	public void OutToExcel(String studentNumber,@ModelAttribute("classInfoId") ClassInfo classInfoId,String studentName,String birthday,String telephone, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(studentNumber == null) studentNumber = "";
        if(studentName == null) studentName = "";
        if(birthday == null) birthday = "";
        if(telephone == null) telephone = "";
        List<Student> studentList = studentService.queryStudent(studentNumber,classInfoId,studentName,birthday,telephone);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Student信息记录"; 
        String[] headers = { "学号","所在班级","姓名","性别","学生照片","出生日期","政治面貌","联系电话"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<studentList.size();i++) {
        	Student student = studentList.get(i); 
        	dataset.add(new String[]{student.getStudentNumber(),student.getClassInfoId().getClassName(),student.getStudentName(),student.getSex(),student.getPhotoUrl(),student.getBirthday(),student.getZzmm(),student.getTelephone()});
        }
        /*
