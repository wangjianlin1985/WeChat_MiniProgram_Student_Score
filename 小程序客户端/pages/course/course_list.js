...
Page({
  ...
  data: {
    ...
    teacherName: "", //任课教师查询关键字
    coursePlace: "", //上课教室查询关键字
    courses: [], //界面显示的课程信息列表数据
  ...
  listCourse: function () {
    var self = this
    var url = config.basePath + "api/course/list"
    //如果要显示的页码超过总页码不操作
    ...
    utils.sendRequest(url, {
      ...
      setTimeout(function () {  
        self.setData({
          courses: self.data.courses.concat(res.data.list),
  ...
  removeCourse: function (e) {
    var self = this
    var courseNo = e.currentTarget.dataset.courseno
    wx.showModal({
      title: '提示',
      content: '确定要删除courseNo=' + courseNo + '的记录吗？',
      success: function (sm) {
        if (sm.confirm) {
          // 用户点击了确定 可以调用删除方法了
          var url = config.basePath + "api/course/delete/" + courseNo
