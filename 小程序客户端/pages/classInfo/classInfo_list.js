...
Page({
  ...
  listClassInfo: function () {
    ...
    utils.sendRequest(url, {
      "classNo": self.data.classNo,
      "className": self.data.className,
      "banzhuren": self.data.banzhuren,
  ...
  removeClassInfo: function (e) {
    var self = this
    var classNo = e.currentTarget.dataset.classno
    wx.showModal({
      title: '提示',
      content: '确定要删除classNo=' + classNo + '的记录吗？',
      success: function (sm) {
        if (sm.confirm) {
          // 用户点击了确定 可以调用删除方法了
          var url = config.basePath + "api/classInfo/delete/" + classNo
          utils.sendRequest(url, null, function (res) {
            ...
            for (var i = 0; i < classInfos.length; i++) {
              if (classInfos[i].classNo == classNo) {
