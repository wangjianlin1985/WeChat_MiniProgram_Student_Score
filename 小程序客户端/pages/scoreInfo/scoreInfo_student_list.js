...
Page({
  ...
  data: {
    ...
    studentNumber_Index: "0", //图书分类查询条件
    students: [{ "studentNumber": "", "studentName": "不限制" }],
    courseNo_Index: "0", //图书分类查询条件
    courses: [{ "courseNo": "", "courseName": "不限制" }],
  ...
  listScoreInfo: function () {
    var self = this
    var url = config.basePath + "api/scoreInfo/student_list"
    //如果要显示的页码超过总页码不操作
    ...
    utils.sendRequest(url, {
      "studentNumber.studentNumber": self.data.students[self.data.studentNumber_Index].studentNumber,
      "courseNo.courseNo": self.data.courses[self.data.courseNo_Index].courseNo,
      "page": self.data.page,
  ...
  init_query_params: function () {
    ...
    utils.sendRequest(url, null, function (res) {
      wx.stopPullDownRefresh();
      self.setData({
        courses: self.data.courses.concat(res.data),
      })
    })
  },
